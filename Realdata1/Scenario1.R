...
covtype1 = read.table('covtype.data.txt',header=FALSE,sep=',')
colnames(covtype1)
head(covtype1)
covtype = covtype1[covtype1[,'V55'] == 6| covtype1[,'V55'] ==4,]
unique(which(is.na(covtype),arr.ind=TRUE)[,1])
Y = covtype[,'V55'];Y = ifelse(Y == 4, 1,0); mean(Y,na.rm = TRUE);#sum(Y)
X = as.matrix(covtype[,c(1:10)])
#X <- scale(X) #t(t(X) / apply(X, 2, sd))
X <- cbind(1,X)
set.seed(20230120)
re=300; t.n = length(Y); m1 = 10;deg = 5; batch = 100; pilot <- 150
...
for(i in 1:re)
{
   ...
   { 
      ...
      print(rbind(  t.beta,set.ent.IMP$c.est, set.ses.IMP$c.est))
      print(j)
   }
   print(i)
}
save.image("Realcase1.RData")
...
plot(tab1, ylim = c(min(tab4),max(tab1)) ,xaxt = "n",  cex.lab=1.6, lty=2,
