...
SES <- function(pool.x, pool.y, sub.x, sub.y, sub.p, batch, c.beta, i.model, setup = FALSE, method = 'Self') 
{
   if(setup == TRUE)
   {
      ...
      imp.v = predict(i.model,newdata=data.frame(x2=pool.x[,2],x3=pool.x[,3],x4=pool.x[,4],x5=pool.x[,5],x6=pool.x[,6],
                                     x7=pool.x[,7],x8=pool.x[,8],x9=pool.x[,9],x10=pool.x[,10],x11=pool.x[,11]),type="response")
...
m.t <- function(sub.y, sub.x, res.x, pi, deg, method = 'IPW', design){
   ...
   if(method == 'IMP'){
     ...
     x8 = sub.x[,8];x9=sub.x[,9];x10=sub.x[,10];x11=sub.x[,11]
     ...
     Imp.y = predict(reg,newdata=data.frame(x2=t.x[,2],x3=t.x[,3],x4=t.x[,4],x5=t.x[,5],x6=t.x[,6],
                                           x7=t.x[,7],x8=t.x[,8],x9=t.x[,9],x10=t.x[,10],x11=t.x[,11]),type="response")
