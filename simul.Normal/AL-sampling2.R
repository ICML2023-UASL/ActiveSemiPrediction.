...
mod.Ent.noW <- mod.SES.noW <- mod.Ent.CW <- mod.SES.CW  <- mod.Ent.IMP <- mod.SES.IMP <- array(0, dim=c(m1,length(beta),re))
...
for(i in 1:re)
{
  ...
  #inital
  set.ent.noW <-entropy(pool.X, pool.Y, pilot.X, pilot.Y, sub.p.u, batch, pilot.beta, setup = TRUE) 
  set.ses.noW <-SES(pool.X, pool.Y, pilot.X, pilot.Y, sub.p.u, batch, pilot.beta, i.model = imp.moe, setup = TRUE) 
  ...
  { 
    ##Sampling
    set.ent.noW = entropy(set.ent.noW$pool.x, set.ent.noW$pool.y, set.ent.noW$sub.x, set.ent.noW$sub.y, set.ent.noW$sub.p, batch, set.ent.noW$c.est) 
    set.ses.noW = SES(set.ses.noW$pool.x, set.ses.noW$pool.y, set.ses.noW$sub.x, set.ses.noW$sub.y, set.ses.noW$sub.p, batch, set.ses.noW$c.est, set.ses.noW$i.model)
    ...
    ##Model training
    #Entropy 
    mod.Ent.noW[j,,i] <- m.t(set.ent.noW$sub.y, set.ent.noW$sub.x, set.ent.noW$pool.x, set.ent.noW$pi, deg, method = 'No')$cur.est
    #SES Mean
    mod.SES.noW[j,,i] <- set.ses.noW$c.est <- m.t(set.ses.noW$sub.y, set.ses.noW$sub.x, set.ses.noW$pool.x, set.ses.noW$pi, deg, method = 'No')$cur.est
    set.ses.noW$i.model <- m.t(set.ses.noW$sub.y, set.ses.noW$sub.x, set.ses.noW$pool.x, set.ses.noW$pi, deg, method = 'IMP')$i.model
    ...
    print(rbind(  t.beta[i,], set.ses.noW$c.est, set.ses.CW$c.est, set.ses.IMP$c.est))
    print(j)
  }
  print(i)
}
save.image("Case2.RData")

m.t.beta = colMeans(t.beta[1:re,])
s= 10
m.t.beta
rowMeans(mod.Ent.noW[s,,1:re])
round(rowMeans(mod.SES.noW[s,,1:re]),4)
...
pdf(file = "~/Desktop/RPlot04.pdf", width = 8, height = 5)
par(mar = c(5.1, 6.1, 4.1, 1.1))
plot(tab2, ylim = c(-0.001,max(tab2)) ,xaxt = "n",  cex.lab=2.5, lty = 6,
     col = 'blue',cex.axis=2,lwd= 4,#,main = expression(F[1])
     cex.main=2, xlab='Batch',ylab = '', type = 'b', pch = 4,  cex = 2)
