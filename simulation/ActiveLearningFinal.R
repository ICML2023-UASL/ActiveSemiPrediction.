...
Datageneration = function(n, beta, corr, Dist = 'mzNormal', link = 'mod1')
{
   ...
   #if(link == 'mod2')
   #{
   #   prob = 1 - 1 / (1 + exp(c(X %*% beta + 1 * X[,2] * X[,3] + 1 * X[,5] * X[,7] + 1 * X[,4] * X[,8])))  
      #prob = 1 - 1 / (1 + exp(c(X %*% beta - 0.5 * X[,4]^2 -  0.5 *X[,7]^2 + X[,2] * X[,3] +  X[,5] * X[,7] - X[,4] * X[,8])))  
...
##############active entropy####
SES <- function(pool.x, pool.y, sub.x, sub.y, sub.p, batch, c.beta, i.model, setup = FALSE, method = 'Self') 
{
   if(setup == TRUE)
   {
...
m.t <- function(sub.y, sub.x, res.x, pi, deg, method = 'IPW', design){
   ...
   if(method == 'IMP'){
     ...
     #gbm.fit1 <- gbm(formula = sub.y ~ .,data=data1, distribution = "bernoulli")  
     #Imp.y <- suppressMessages(predict(object=gbm.fit1, data.frame(t.x), type='response'))
